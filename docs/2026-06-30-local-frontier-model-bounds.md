...
These check whether the conceptual scaffolding is in place.

1. State the Resident-Flux Feasibility Theorem. Why is the feasible region a
   rectangle, and why is $CR$ its natural measure rather than $C + RT$?
2. The Dennard Decode Bound and the KV-aware bound are both upper bounds on the
   same quantity. Which is tighter, and exactly which term does the looser one
   discard?
3. Why must reserved context $L_{\mathrm{alloc}}$ and active context
   $L_{\mathrm{read}}$ be tracked separately? Name the part of the bound each one
   controls.
4. Explain why the memory-fit batch $b_{\mathrm{mem}}$ alone produces the
   "hundred sessions" illusion, and how the rate floor $r_\star$ removes it.
5. For a dense model, why do $W_{\mathrm{resident}}$ and $W_{\mathrm{batch}}(b)$
   coincide, while for an MoE model they do not?
6. Why is multiplying throughput by the acceptance factor $\rho$ unsafe unless
   the draft-model cost is added to the denominator?

## Exercises

These exercise the machinery on small instances.

1. *Fit.* A model has $W_{\mathrm{resident}} = 60$ GB and
   $K_{\mathrm{alloc}} = 1.2$ GB/session. On DGX Spark ($C = 128$ GB, $O = 8$ GB),
   compute $b_{\mathrm{mem}}$.
2. *Expert saturation.* For the MoE diversity factor $m(b\rho) = E(1 -
   (1 - k/E)^{b\rho})$, evaluate $m$ at $b\rho = 1$ and show it equals $k$ (one
   routing touches $k$ experts), and show $m \to E$ as $b\rho \to \infty$ (a large
   batch touches every expert). What does the second limit imply about whether MoE
   batching amortizes weight traffic for free?
3. *Corollary.* Starting from the KV-aware bound, derive the Dennard Bound by
   dropping $K_{\mathrm{read}}$ and applying the capacity cap on $b$. Identify the
   step where the private traffic is discarded.
4. *Rate floor.* Using $r(b) \ge r_\star$, derive $b_{\mathrm{rate}}$, and explain
   why it falls as $L_{\mathrm{read}}$ grows while $b_{\mathrm{mem}}$ does not.
5. *Sanity check.* For the Gemma row ($b^\star = 16$, aggregate $\le 333$ tok/s),
   compute the per-session ceiling and confirm it clears the default floor
   $r_\star = 20$. Then explain why $b = 32$ and $b = 64$ are rejected.
6. *Single session.* Write the $b = 1$ bound for a dense model and explain, in
   one sentence, why capacity appears only as a fit gate and not in the rate.

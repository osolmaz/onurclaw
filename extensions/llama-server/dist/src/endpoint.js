...
export function resolveLlamaServerEndpoint(configuredBaseUrl) {
...
export function normalizeLlamaServerProviderConfig(provider) {
    const endpoint = resolveLlamaServerEndpoint(provider.baseUrl);
    const request = provider.request ?? {};
    const normalizedRequest = typeof request.allowPrivateNetwork === "boolean"
        ? request

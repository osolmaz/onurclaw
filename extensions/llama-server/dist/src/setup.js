import { applyAuthProfileConfig, buildApiKeyCredential, ensureApiKeyFromEnvOrPrompt, normalizeOptionalSecretInput, updateAuthProfileStoreWithLock, upsertAuthProfileWithLock, } from "openclaw/plugin-sdk/provider-auth";
...
import { hasLlamaServerAuthorizationHeader, resolveLlamaServerProviderHeaders, resolveLlamaServerRuntimeApiKey, } from "./auth.js";
import { LLAMA_SERVER_DEFAULT_API_KEY_ENV_VAR, LLAMA_SERVER_DEFAULT_ORIGIN, LLAMA_SERVER_PROVIDER_ID, LLAMA_SERVER_PROVIDER_LABEL, } from "./defaults.js";
...
function stripCredentialOverrides(provider) {
    if (!provider) {
        return provider;
    }
    const headers = Object.fromEntries(Object.entries(provider.headers ?? {}).filter(([name]) => name.toLowerCase() !== "authorization"));
    return {
        ...provider,
        auth: undefined,
        apiKey: undefined,
...
function hasEndpointChanged(provider, baseUrl) {
    ...
    return (resolveLlamaServerEndpoint(configuredBaseUrl).inferenceBaseUrl !==
        resolveLlamaServerEndpoint(baseUrl).inferenceBaseUrl);
}
function removeLlamaServerAuthProfileConfig(config) {
    const profiles = Object.fromEntries(Object.entries(config.auth?.profiles ?? {}).filter(([id]) => id !== PROFILE_ID));
    const order = Object.entries(config.auth?.order ?? {}).reduce((nextOrder, [providerId, providerOrder]) => {
            ...
            ? [
                {
                    ...
                    credential: buildApiKeyCredential(LLAMA_SERVER_PROVIDER_ID, params.credentialInput, undefined, { config: params.config }),
                },
            ]
            : [],
...
export async function detectLlamaServerSetup(ctx) {
...
export async function prepareLlamaServerSetup(ctx) {
    const provider = ctx.config.models?.providers?.[LLAMA_SERVER_PROVIDER_ID];
    let discovery;
    try {
        discovery = await discoverForSetup({
...
export async function runLlamaServerSetup(ctx) {
...
async function validateNonInteractiveDiscovery(ctx) {
    ...
    const baseUrl = normalizeOptionalSecretInput(ctx.opts.customBaseUrl) ??
    ...
    if (!modelId || !discovery.models.some((model) => model.config.id === modelId)) {
        ...
        ctx.runtime.error(requestedModelId
...
export async function validateLlamaServerNonInteractive(ctx) {
    return Boolean(await validateNonInteractiveDiscovery(ctx));
}
/** Non-interactive setup with optional API-key persistence. */
export async function configureLlamaServerNonInteractive(ctx) {

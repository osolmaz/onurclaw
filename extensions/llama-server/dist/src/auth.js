import { CUSTOM_LOCAL_AUTH_MARKER, hasConfiguredSecretInput, isNonSecretApiKeyMarker, normalizeOptionalSecretInput, } from "openclaw/plugin-sdk/provider-auth";
import { resolveApiKeyForProvider } from "openclaw/plugin-sdk/provider-auth-runtime";
import { resolveConfiguredSecretInputString } from "openclaw/plugin-sdk/secret-input-runtime";
import { LLAMA_SERVER_LOCAL_AUTH_MARKER, LLAMA_SERVER_PROVIDER_ID } from "./defaults.js";
export function hasLlamaServerAuthorizationHeader(headers) {
    if (!headers || typeof headers !== "object" || Array.isArray(headers)) {
        return false;
    }
    return Object.entries(headers).some(([name, value]) => name.trim().toLowerCase() === "authorization" && hasConfiguredSecretInput(value));
}
export function shouldUseLlamaServerSyntheticAuth(providerConfig) {
    const apiKey = normalizeOptionalSecretInput(providerConfig?.apiKey)?.trim();
    const hasRealApiKey = hasConfiguredSecretInput(providerConfig?.apiKey) &&
        apiKey !== LLAMA_SERVER_LOCAL_AUTH_MARKER &&
        apiKey !== CUSTOM_LOCAL_AUTH_MARKER;
    return !hasRealApiKey;
}
export function buildLlamaServerAuthHeaders(apiKey, configuredHeaders) {
    const headers = {

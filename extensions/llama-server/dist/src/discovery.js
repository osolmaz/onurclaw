...
import { LLAMA_SERVER_DISCOVERY_CACHE_TTL_MS, LLAMA_SERVER_DISCOVERY_TIMEOUT_MS, } from "./defaults.js";
import { resolveLlamaServerEndpoint } from "./endpoint.js";
import { mapLlamaServerModel, } from "./models.js";
...
function cacheDiscovery(key, entry, now) {
...
export function clearLlamaServerDiscoveryCacheForTests() {
...
function readModelRows(body) {
    ...
    return data.filter((entry) => Boolean(entry) && typeof entry === "object" && !Array.isArray(entry));
}
function shouldReadProps(row) {
...
export async function discoverLlamaServer(params) {
    ...
    const hasCredentialScope = Boolean(normalizedApiKey && !isNonSecretApiKeyMarker(normalizedApiKey)) ||
    ...
    const health = healthResult.kind === "response" && healthResult.status === 200
        ? "ready"
        : healthResult.kind === "response" && healthResult.status === 503
            ? "loading"
            : "unknown";
    if (healthResult.kind === "response" &&
    ...
    for (let offset = 0; offset < propsRowIndexes.length; offset += LLAMA_SERVER_ROUTER_PROPS_CONCURRENCY) {
        const remainingMs = propsDeadline - Date.now();
        if (remainingMs <= 0) {
            break;
        }
        const results = await Promise.all(propsRowIndexes
            ...
            return props ? [index, props] : undefined;
        }));
    ...
    const result = { kind: "success", endpoint, health, models, fetchedAt };
    if (cacheTtlMs > 0) {

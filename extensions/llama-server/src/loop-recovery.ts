...
export const LOOP_RECOVERY_TERMINAL_TEXT =
  "I couldn't safely continue after the same tool call produced no new evidence. Please provide the missing access or clarify how you want me to proceed.";

export const DEFAULT_LOOP_RECOVERY_INSTRUCTION =
...
export type LlamaServerLoopRecoveryConfig = {
  enabled: boolean;
...
export function resolveLlamaServerLoopRecoveryConfig(
  pluginConfig: unknown,
): LlamaServerLoopRecoveryConfig {
  const raw =
    isRecord(pluginConfig) && isRecord(pluginConfig.loopRecovery)
      ? (pluginConfig.loopRecovery as RawLoopRecoveryConfig)
      : {};
  const repeatThreshold =
    typeof raw.repeatThreshold === "number" &&
...
function textOutcome(result: ToolResultMessage): string | undefined {
  const parts: string[] = [];
...
function extractCycle(messages: Message[], end: number): ToolCycle | undefined {
  const result = messages[end];
  const assistant = messages[end - 1];
  if (result?.role !== "toolResult" || assistant?.role !== "assistant") {
    return undefined;
  }
  const calls = assistant.content.filter((entry): entry is ToolCall => entry.type === "toolCall");
  if (calls.length !== 1) {
    return undefined;
  }
  const call = calls[0];
...
function findRepeatedTail(messages: Message[], config: LlamaServerLoopRecoveryConfig): ToolCycle[] {
...
function annotateRetainedResult(result: ToolResultMessage, repeatCount: number): ToolResultMessage {
  const note = {
    type: "text" as const,
...
export function recoverRepeatedToolLoop(params: {
  ...
  config: LlamaServerLoopRecoveryConfig;
  now?: number;
  instruction?: string;
}): LoopRecoveryResult {
...
function pushTextResponse(
  ...
  const partial = { ...message, content: [] } satisfies AssistantMessage;
...
export function guardToollessRecoveryStream(params: {
  model: Model;
  source: Awaited<ReturnType<StreamFn>> | ReturnType<StreamFn>;
}) {
